...
def get_ai_hint(target_type, target_value):
    ...
    # Жесткий промпт с системой ограничений
    if "страну" in target_type.lower():
        context_rules = """
        ЗАПРЕЩЕНО: 
        - Упоминать географическое положение (соседи, части света, близость к морям/горам).
        - Называть столицы, флаги, валюту или общеизвестные символы.
        - Упоминать поп-культурных исторических личностей (цари, президенты, великие полководцы).
        - Давать очевидные факты из школьной программы.
        ФОКУС: Глубокая этимология названия региона, уникальные древние винодельческие законы, геологические аномалии почв (периоды юры или мела) или забытые традиции предков.
        """
    else:
        context_rules = """
        ЗАПРЕЩЕНО:
        - Описывать вкус, аромат, цвет или любые органолептические свойства.
        - Называть историческую родину сорта или текущие популярные регионы его выращивания.
        - Давать советы по гастрономии.
        ФОКУС: Ботанические особенности лозы (форма листа, тип цветения), генетическое происхождение (кто родители сорта), этимология названия на мертвых языках или редкие ампелографические факты.
        """

    prompt = f"""
    Напиши один профессиональный факт про {target_type} '{target_value}'.
    
    {context_rules}

    ОБЩИЕ ПРАВИЛА:
    1. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО называть само слово '{target_value}' или его части.
    2. ЗАПРЕЩЕНО врать или выдумывать. Используй только научно подтвержденные или исторически задокументированные данные.
    3. Только 2-3 полных, законченных предложения. 
    4. Начни ответ СРАЗУ с текста факта, без вступлений.
    """
    
    try:
        # Увеличиваем параметры для стабильности текста
        response = model.generate_content(
            prompt,
            generation_config=genai.types.GenerationConfig(
                temperature=0.4, # Снижаем температуру для точности и отсутствия "бреда"
                max_output_tokens=600, # Даем большой запас, чтобы не обрезал
                candidate_count=1
            )
        )
        ...
        # Финальная проверка на обрыв слова (если точка не стоит в конце)
        if not hint_text.endswith(('.', '!', '?')):
            # Если текст обрезался, пробуем убрать последнее незаконченное предложение
            last_dot = hint_text.rfind('.')
            if last_dot != -1:
                hint_text = hint_text[:last_dot+1]
