...
def get_ai_hint(target_type, target_value):
    ...
    # Новый промпт с упором на факты и историю
    prompt = f"""
    Ты — эрудированный историк вина и сомелье. Мы играем в винное казино.
    Дай один поразительный, редкий или просто очень интересный историко-географический факт про {target_type} '{target_value}'.
    
    ПРАВИЛА:
    1. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО называть само слово '{target_value}'.
    2. ЗАПРЕЩЕНО описывать вкус, аромат, цвет или органолептику (никаких 'ноток вишни' или 'высокой кислотности').
    3. ФАКТ должен быть проверяемым и реальным (не выдумывай!).
    4. Фокусируйся на: уникальных законах, рекордах (самый старый, самый высокий), этимологии названия (не называя само слово), связи с великими личностями или климатических аномалиях региона.
    5. Избегай банальностей типа флагов или формы границ.
    6. Будь краток: 1-2 предложения.
    """
    
    try:
        # Устанавливаем настройки генерации для точности и креативности
        response = model.generate_content(
            prompt,
            generation_config=genai.types.GenerationConfig(
                temperature=0.8, # Больше разнообразия в фактах
                top_p=0.95,
