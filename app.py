...
import random # Добавь это в начало файла к остальным импортам
...
def get_ai_hint(target_type, target_value):
    ...
    # Генерируем случайное число для уникальности каждого ответа
    seed = random.randint(1, 100000)
    
    if "страну" in target_type.lower():
        rules = """
        - ЗАПРЕЩЕНО упоминать части света, соседей, горы, моря, реки и любую географическую привязку.
        - ЗАПРЕЩЕНО называть столицы, флаги и известных правителей.
        - ФОКУС: Глубокая этимология, уникальные типы почв (геологические пласты), древние археологические находки или забытые винные законы прошлого.
        """
    else:
        rules = """
        - ЗАПРЕЩЕНО описывать вкус, запах, цвет.
        - ЗАПРЕЩЕНО называть родину сорта или страны, где он растет сейчас.
        - ФОКУС: Ботаника (генетика, форма листа, восковой налет), происхождение названия из мертвых языков, легенды, не связанные с географией.
        """

    prompt = f"""
    Напиши один профессиональный и очень редкий факт про {target_type} '{target_value}'.
    Уникальный идентификатор запроса: {seed}

    ПРАВИЛА:
    1. НЕ называй само слово '{target_value}'.
    2. {rules}
    3. Только достоверные данные. Если факта нет — расскажи о геологии почв этого региона.
    4. Напиши ровно 2-3 полных предложения. 
    5. Начни СРАЗУ с факта. Без вступлений.
    """
    
    try:
        # Устанавливаем высокую точность и достаточный лимит символов
        response = model.generate_content(
            prompt,
            generation_config=genai.types.GenerationConfig(
                temperature=0.7, # Чуть выше для разнообразия фактов
                max_output_tokens=800, # Увеличили до максимума
            )
        )
        
        hint_text = response.text.strip().replace('"', '')
        
        # ЛОГИКА ПРОТИВ ОБРЕЗАНИЯ СЛОВ:
        # Если в конце нет знака препинания, значит текст оборван. 
        # Отрезаем всё после последней найденной точки.
        if not hint_text.endswith(('.', '!', '?')):
            last_dot = max(hint_text.rfind('.'), hint_text.rfind('!'), hint_text.rfind('?'))
            if last_dot != -1:
