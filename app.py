...
def get_ai_hint(target_type, target_value):
    ...
    # Максимально открытый промпт для интеллектуальных подсказок
    prompt = f"""
    Напиши один сложный, неочевидный и профессиональный факт про {target_type} '{target_value}'.
    
    УСЛОВИЯ:
    1. НЕ называй само слово '{target_value}'.
    2. НЕ описывай вкус, аромат или цвет. 
    3. Сфокусируйся на терруаре, геологии, уникальных методах подрезки лозы, исторических прецедентах или этимологии, уходящей в глубокое прошлое.
    4. Факт должен быть таким, чтобы его мог знать опытный сомелье или историк, но не рядовой любитель. 
    5. Напиши 2-4 полноценных предложения. Дай мысли "воздуха", не обрывай на полуслове.
    6. Не используй никаких вступлений, приветствий или обращений. Только текст факта.
    """
    ...
    try:
        response = model.generate_content(
            ...
            generation_config=genai.types.GenerationConfig(
                temperature=0.85, # Баланс между точностью и полетом мысли
                max_output_tokens=500, # Даем много места для развернутого ответа
            )
        )
        
        result = response.text.strip()
        
        # Убираем кавычки, если он их добавит по привычке
        if result.startswith('"') and result.endswith('"'):
            result = result[1:-1]
            
        return result
    except Exception as e:
        return f"Произошла заминка при поиске факта: {str(e)}"
