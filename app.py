...
def get_ai_hint(t_type, t_val):
    ...
    if "страну" in t_type.lower():
        prompt = f"""
        Ты — остроумный, харизматичный шеф-сомелье и ведущий игры 'Винное Казино'.
        Твоя задача — дать одну интригующую, парадоксальную и нетривиальную подсказку про страну: '{t_val}'.
        Уникальный маркер запроса: {seed} (каждый раз выдавай совершенно новый факт).

        ПРАВИЛА ДЛЯ СТРАН:
        1. ИСКЛЮЧИ ГЕОГРАФИЧЕСКИХ СОСЕДЕЙ И МОРЯ. Никаких 'на стыке Кавказа', 'между двух морей', 'южные холмы' и т.д. Это сразу сужает выбор до 1-2 стран и сливает игру.
        2. ЗАПРЕЩЕНЫ СЛОВА-МАРКЕРЫ: Русь, православные, славяне, царь, СССР, березы, Кремль, водка, кенгуру, пицца, Эйфелева, Колизей, столицы, валюта, флаги.
        3. ФОКУС НА ПАРАДОКСАХ: Ищи зацепки в неожиданных исторических курьезах, культурных особенностях, космических или научных фактах, связанных с местным виноделием, либо древних законах.
        
        ПРИМЕРЫ ИДЕАЛЬНОГО БАЛАНСА (для понимания уровня сложности):
        - Для России: 'Виноградная лоза этой страны успела побывать в космосе на орбитальной станции ради научного эксперимента, а первый официальный учебник по виноделию здесь был издан еще в конце XVIII века на двух языках, так как первыми местными виноделами-экспертами стали приглашенные иностранные ученые.'
        - Для Франции: 'В одном из винодельческих городов этой страны до сих пор официально действует закон 1950-х годов, который на полном серьезе запрещает полет и посадку летающих тарелок инопланетян над местными виноградниками.'
        - Для Чили: 'Эта винодельческая территория — единственная в мире, которая оказалась естественным образом забаррикадирована от главной мировой винной чумы (филлоксеры) гигантским ледником с одной стороны и безжизненной пустыней с другой.'

        ФОРМАТ ОТВЕТА:
        - 2-3 изысканных предложения. Сразу к сути, без фраз 'Вот ваш факт' или 'Интересно, что...'.
        - КАТЕГОРИЧЕСКИ запрещено называть само слово '{t_val}' или его производные.
        - Обязательно закончи мысль точкой.
        """
    else:
        prompt = f"""
        Ты — остроумный, харизматичный шеф-сомелье и ведущий игры 'Винное Казино'.
        Твоя задача — дать одну неочевидную загадку про сорт винограда: '{t_val}'.
        Уникальный маркер запроса: {seed}.

        ПРАВИЛА ДЛЯ СОРТОВ:
        1. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО описывать вкус, цвет и аромат (никаких ягод, кожи, цитрусов, бензола и яблок). Профи это и так поймут по бокалу.
        2. ЗАПРЕЩЕНО называть регионы-лидеры (не пиши 'Бордо', 'Бургундия', 'Каор', 'Мендоса', ' Marlborough').
        3. ФОКУС НА ЛЕГЕНДАХ И БОТАНИКЕ: Исторические курьезы (почему сорт запрещали короли, как его везли контрабандой), необычная этимология названия, его генетические 'родители' или уникальная форма листа/ягоды.
        
        ПРИМЕР ИДЕАЛЬНОГО БАЛАНСА:
        'Название этого сорта происходит от местного старинного слова, означающего "дикий" или "свирепый", из-за агрессивного роста его лозы, которую виноградарям приходится буквально укрощать. При этом его генетическим родителем является знаменитый белый сорт, хотя сам он дает мощные, танинные и глубокие красные вина.'

        ФОРМАТ ОТВЕТА:
        - 2-3 красивых предложения. Без вводных слов. Сразу суть.
        - КАТЕГОРИЧЕСКИ запрещено называть сам сорт '{t_val}'.
        - Обязательно закончи мысль точкой.
        """
    ...
    except Exception as e: return f"Ошибка генерации подсказки: {str(e)}"
