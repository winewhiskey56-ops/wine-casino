...
def get_ai_hint(t_type, t_val):
    ...
    if "страну" in t_type.lower():
        prompt = f"""
        Ты — сомелье-историк. Твоя задача — дать ОДНУ сложную подсказку про страну: '{t_val}'.
        ...
        {{"text": "Здесь пиши строго 2 полных, развернутых предложения без сокращений. Сразу факты, без приветствий. Не называй слово '{t_val}'."}}
        """
    else:
        prompt = f"""
        Ты — сомелье-ампелограф. Твоя задача — дать ОДНУ сложную подсказку про сорт винограда: '{t_val}'.
        Уникальный маркер: {seed}.
        ...
        3. ФОКУС: Происхождение названия (этимология), ботанические особенности лозы или листа, исторические легенды и родственные связи в семействе лоз. Избегай слов 'мутация' и 'генетика'.

        ТРЕБОВАНИЯ К ФОРМАТУ:
        Выдай ответ строго в формате JSON:
        {{"text": "Здесь пиши строго 2 полных, развернутых предложения без сокращений. Сразу суть, без пафоса. Не называй сорт '{t_val}'."}}
        """
    ...
    try:
        ...
        # Переключаем модель на выдачу JSON
        resp = model.generate_content(
            prompt, 
            generation_config=genai.types.GenerationConfig(
                temperature=0.5, # Ещё немного снизили для максимальной стабильности
        ...
        # Парсим чистый текст из JSON
        raw_json = json.loads(resp.text.strip())
        res = raw_json.get("text", "").strip().replace('"', '')
        
        if not res.endswith(('.', '!', '?')):
            res += "."
        return res
    except Exception as e: 
        # Если JSON-парсинг сорвался, выдаем как обычный текст
        try:
            res = resp.text.strip().replace('"', '')
            return res if res.endswith(('.', '!', '?')) else res + "."
        except:
            return "ИИ отвлекся на дегустацию. Пожалуйста, нажмите кнопку обновления еще раз."
