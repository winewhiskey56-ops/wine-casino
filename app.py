...
def get_ai_hint(target_type, target_value):
    ...
    # Скорректированный промпт для полноценных фактов
    prompt = f"""
    Ты — эрудированный винный историк. Мы играем в винное казино.
    Твоя задача: рассказать один захватывающий и редкий исторический или географический факт про {target_type} '{target_value}'.
    
    ПРАВИЛА:
    1. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО называть само слово '{target_value}' или производные от него.
    2. ЗАПРЕЩЕНО описывать вкус (кислинку, сладость), ароматы (фрукты, цветы) или цвет. Участники уже пьют это вино.
    3. ФАКТ должен быть развернутым (минимум 15-20 слов, но не более 50). Не отвечай двумя словами!
    4. Темы: Этимология названия, древние легенды, уникальные законы этого региона, связь с королями или войны, которые повлияли на эти виноградники.
    5. Будь загадочным, но информативным. Игроки должны извлечь из твоего рассказа зацепку, чтобы угадать ответ.
    """
    ...
    try:
        response = model.generate_content(
            ...
            generation_config=genai.types.GenerationConfig(
                ...
                top_p=1.0,
                max_output_tokens=250, # Увеличиваем лимит символов на выход
            )
        )
        # Очищаем ответ от лишних кавычек, если ИИ их добавит
