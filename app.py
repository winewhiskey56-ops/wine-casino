...
def get_ai_hint(t_type, t_val):
    ...
    if "страну" in t_type.lower():
        prompt = f"""
        Ты — профессиональный сомелье-историк. Твоя задача — дать ОДНУ сложную, но интересную подсказку-загадку про страну: '{t_val}'.
        ...
        3. ФОКУС: Исторические курьезы, наука, космос, необычные законы или гастрономические парадоксы.

        ПРАВИЛА ОФОРМЛЕНИЯ (КРИТИЧЕСКИ ВАЖНО):
        - Напиши строго 2 или 3 ПОЛНЫХ, развернутых предложения. 
        - Говори строго фактами. Никаких театральных вступлений вроде 'Приготовьтесь, господа', 'Ох, мои дорогие знатоки' или 'Перед вами страна'. Сразу пиши факт.
        - Никаких сокращений слов. Не называй слово '{t_val}'.
        """
    else:
        prompt = f"""
        Ты — профессиональный сомелье-ампелограф. Твоя задача — дать ОДНУ сложную подсказку-загадку про сорт винограда: '{t_val}'.
        Уникальный маркер: {seed}.
        ...
        3. ФОКУС: Происхождение названия (этимология), ботанические особенности лозы или листа, исторические курьезы и мифы.

        ПРАВИЛА ОФОРМЛЕНИЯ (КРИТИЧЕСКИ ВАЖНО):
        - Напиши строго 2 или 3 ПОЛНЫХ, развернутых предложения.
        - Пиши строго по делу, без пафоса и вступлений вроде 'Я — благородный сорт' или 'Слушайте внимательно'. Сразу суть.
        - Никаких сокращений слов. Не называй сорт '{t_val}'.
        """
    ...
    try:
        ...
        # Снизили температуру до 0.7 для стабильности и убрали лишний разброс (top_p)
        ...
        # Если ИИ всё равно умудрился бросить фразу недописанной, страхуем вывод
        if not res.endswith(('.', '!', '?')):
            res += "..."
        return res
