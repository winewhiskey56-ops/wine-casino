...
def get_ai_hint(t_type, t_val):
    ...
    if "страну" in t_type.lower():
        ...
        3. ФОКУС НА ПАРАДОКСАХ: Ищи зацепки в исторических курьезах, космосе, науке или необычных законах.
        
        ВАЖНОЕ ПРАВИЛО ФОРМАТА:
        - Напиши ровно 2-3 ПОЛНЫХ, развернутых, законченных предложения. 
        - КАТЕГОРИЧЕСКИ запрещено использовать любые сокращения (НЕ пиши 'в.', 'г.', 'в т.ч.'). Пиши слова полностью!
        - Не ставь точки внутри предложений. Только в самом конце законченной мысли.
        - Сразу к сути, без вводных фраз. Не называй слово '{t_val}'.
        """
    else:
        prompt = f"""
        Ты — остроумный сомелье и ведущий игры 'Винное Казино'.
        ...
        - Не ставь точки внутри предложений. Только в самом конце законченной мысли.
        - Сразу к сути, без вводных фраз. Не называй сорт '{t_val}'.
        """
    ...
    try:
        ...
        # Умная проверка финала: если ИИ забыл точку, просто добавляем её
        if not res.endswith(('.', '!', '?')):
            res += "."
        return res
