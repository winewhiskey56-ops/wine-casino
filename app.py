...
def get_ai_hint(t_type, t_val):
    ...
    if "страну" in t_type.lower():
        prompt = f"""
        Ты — остроумный сомелье и ведущий игры 'Винное Казино'.
        Дай одну интригующую, парадоксальную подсказку про страну: '{t_val}'.
        Уникальный маркер запроса: {seed}.

        ПРАВИЛА:
        1. ИСКЛЮЧИ ГЕОГРАФИЧЕСКИХ СОСЕДЕЙ И МОРЯ. Никаких 'на стыке Кавказа', 'между морей'. 
        ...
        3. ФОКУС НА ПАРАДОКСАХ: Ищи зацепки в исторических курьезах, науке, культуре или необычных законах. Пиши простым, но изысканным языком, без пафоса.
        
        ВАЖНОЕ ПРАВИЛО ФОРМАТА:
        - Напиши ровно 2-3 ПОЛНЫХ, развернутых, законченных предложения. 
        - КАТЕГОРИЧЕСКИ запрещено использовать любые сокращения. Пиши слова полностью!
        - Не ставь точки внутри предложений. Только в самом конце законченной мысли.
        - Сразу к сути, без вводных фраз вроде 'Приготовьтесь, господа'. Не называй слово '{t_val}'.
        """
    else:
        prompt = f"""
        Ты — остроумный сомелье и ведущий игры 'Винное Казино'.
        Дай одну неочевидную загадку про сорт винограда: '{t_val}'.
        Уникальный маркер запроса: {seed}.

        ПРАВИЛА:
        1. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО описывать вкус, цвет и аромат (никаких ягод, кожи, цитрусов, бензола).
        2. ЗАПРЕЩЕНО называть регионы-лидеры (Бордо, Бургундия, Каор, Мендоса).
        3. ФОКУС НА ЛЕГЕНДАХ И БОТАНИКЕ: Исторические курьезы, этимология названия, генетика, форма листа.
        
        ВАЖНОЕ ПРАВИЛО ФОРМАТА:
        - Напиши ровно 2-3 ПОЛНЫХ, развернутых, законченных предложения.
        - КАТЕГОРИЧЕСКИ запрещено использовать любые сокращения. Пиши слова полностью!
        - Не ставь точки внутри предложений. Только в самом конце законченной мысли.
        - Сразу к сути, без пафосных вступлений вроде 'Я — прямое потомство'. Не называй сорт '{t_val}'.
        """

    try:
        # Отключаем все фильтры безопасности, чтобы они не обрубали текст на полуслове
        safety_settings = [
        ...
        resp = model.generate_content(
            ...
            generation_config=genai.types.GenerationConfig(
                temperature=1.0, 
                max_output_tokens=800, # увеличили запас токенов
                top_p=0.95
            ),
